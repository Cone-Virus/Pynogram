...
def main():
    ...
    pauseButton= button(725, 240, "images/pauseButton.png")
    pauseScreen = button(255, 115, "images/pauseScreen.png")
    pauseMainMenuButton = button(300, 420, "images/mainmenu.png")
    resumeButton = button(725, 240, "images/resumeButton.png")
    lightMode = button(355, 235, "images/lightModeButton.png")
    darkMode = button(355, 235, "images/darkModeButton.png")

    #Used to determine whether to render darkMode or lightMode button
    darkModeState = 0 #0 for light mode, 1 for dark mode. Toggles when user clicks button
    ...
    while True:
        ...
        if page == "Main Menu":
            ...
            #If gameState = 0, render board every frame. 0 is Default when starting level.
            #If gameState = 1, stop rendering board to prevent flicker,render popup every frame. Board buttons become unusable
            #gameState 2 = pause button. Works the same way as the check solution popup, except different buttons/functionality
            if gameState == 0:
                board.displayBoard(surface) # grid and numbers
                board.displayBoxes(surface) # boxes in the grid
            elif gameState == 1:
                if solCorrect:#Solution Correct, draw victory screen and main menu button
                    puzzleComplete.draw(surface)
                    pcMainMenu.draw(surface)
                else:
                    puzzleIncorrect.draw(surface) #puzzle is incorrect, draw failure screen and try again/show solution buttons
                    tryAgain.draw(surface)
                    showSolution.draw(surface)
            elif gameState == 2:
                pauseScreen.draw(surface)
                pauseMainMenuButton.draw(surface)
                resumeButton.draw(surface)
                if darkModeState == 0: # lightMode is on
                    darkMode.draw(surface) #show darkMode button
                elif darkModeState == 1: #darkMode is on
                    lightMode.draw(surface)
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.MOUSEBUTTONDOWN:
                ...
                if e.button == 1 and muteMusicButton.rect.collidepoint(x, y): # left click on mute music button
                ...
                elif page == "Board" and gameState == 0: #Board phase, user is solving puzzle
                    ...
                    if e.button == 1 and  pauseButton.rect.collidepoint(x,y): #Clicl on pause button
                        gameState = 2

                elif page == "Board" and gameState == 1: #popup phase, User is not currently solving puzzle
                    if solCorrect == False:
                ...
                elif page == "Board" and gameState == 2: #Pause Screen
                    if e.button == 1 and darkMode.rect.collidepoint(x,y): #User clicked on dark mode toggle
                        if darkModeState == 0:
                            darkModeState = 1
                        elif darkModeState == 1:
                            darkModeState = 0
                    if e.button == 1 and pauseMainMenuButton.rect.collidepoint(x,y): #User clicks main menu button
                        page = "Main Menu"
                        gameState = 0
                        canEditGrid = True
                        notNew = True
                        level.difficulty = ""
                        level.solnName = ""
                    if e.button == 1 and resumeButton.rect.collidepoint(x,y): #User clicks pause button again
                        gameState = 0
